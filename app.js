...
function downloadConsult(id){try{let c=data.consultations.find(x=>x.id===id);if(!c){toast('Consultation introuvable');return}let raw=c.foodPlan||emptyFoodPlan(),plan={...emptyFoodPlan(),...raw};Object.keys(plan).forEach(k=>{if(!Array.isArray(plan[k]))plan[k]=[]});let html=`<!doctype html><html lang="fr"><meta charset="utf-8"><title>Consultation fictive - ${esc(c.patient)}</title><style>body{font-family:Arial;padding:40px;color:#172033}h1{color:#1769e0}.box{border:1px solid #ddd;border-radius:12px;padding:16px;margin:14px 0}.pyramid{display:flex;flex-direction:column;align-items:center;gap:3px;margin:20px}.pyr{padding:9px;background:#dfeeff;text-align:center;border-radius:5px}</style><h1>Cabinet Médical Pro</h1><p><b>CONSULTATION — SIMULATION FICTIVE</b></p><div class="box"><b>Patient :</b> ${esc(c.patient)}<br><b>Date :</b> ${fmt(c.date)}<br><b>Médecin :</b> ${esc(c.doctor)}<br><b>Motif :</b> ${esc(c.reason)}</div><div class="box"><b>Constantes</b><p>Température : ${esc(c.temp||'—')} °C • Pouls : ${esc(c.pulse||'—')} bpm • Tension : ${esc(c.bp||'—')} • Saturation : ${esc(c.sat||'—')} %</p></div><div class="box"><b>Observations</b><p>${esc(c.notes||'—')}</p><b>Examen</b><p>${esc(c.exam||'—')}</p><b>Diagnostic fictif</b><p>${esc(c.diagnosis||'—')}</p><b>Traitement fictif</b><p>${esc(c.treatment||'—')}</p><b>Conclusion</b><p>${esc(c.conclusion||'—')}</p></div><div class="box"><h2>Pyramide alimentaire personnalisée</h2>${pyramidDownload(plan)}</div><div class="box"><h2>Journée alimentaire</h2>${mealSummary(plan)}</div><p style="color:#777">Document de jeu — sans valeur médicale.</p></html>`;let blob=new Blob([html],{type:'text/html;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.style.display='none';a.href=url;a.download='consultation-'+c.patient.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/gi,'-').toLowerCase()+'.html';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),2000);toast('Consultation téléchargée')}catch(err){console.error(err);toast('Impossible de télécharger la consultation')}}
